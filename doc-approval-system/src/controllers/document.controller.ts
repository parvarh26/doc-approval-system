...
import { stampApprovedPDF } from '../services/pdf.service';
...
export const getMyDocuments = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        console.error('Fetch my docs error:', error);
        res.status(500).json({ error: 'Error fetching my documents' });
    }
};
...
export const reviewDocument = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (status === 'APPROVED') {
            try {
                stampedPath = await stampApprovedPDF(document.originalPath, document.filename);
            } catch (err) {
...
export const downloadStampedDocument = async (req: Request, res: Response): Promise<void> => {
    try {
        const id = req.params.id as string;
        const document = await prisma.document.findUnique({ where: { id } });
